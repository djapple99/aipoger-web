...
-- Purpose: make 台語熊high a current fixed genre instead of folding it into Original.
-- Safe to run repeatedly; rows already using current labels are ignored.
  ...
  loop
    ...
    execute format(
      ...
      set genre = case genre
        ...
        when '台語熊 High' then '台語熊high'
        when '台語熊 high' then '台語熊high'
        when '台語熊High' then '台語熊high'
        when '台語熊HIGH' then '台語熊high'
        when 'Taiwanese Bear High' then '台語熊high'
        when 'taiwanese bear high' then '台語熊high'
...
'Migrates old AIPOGER genre labels into the current 11-genre taxonomy, where 台語熊high is a fixed genre.';

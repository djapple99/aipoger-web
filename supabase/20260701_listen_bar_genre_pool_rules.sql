-- AIPOGER Bar Heartbreak: genre-pool survival rules.
-- 2026-07-01 current rule:
-- - each music genre has a 36-track public pool
-- - survival and 7-day Honor eligibility start per genre only after that genre reaches 36 public tracks
-- - capacity eviction only removes overflow inside the same genre pool
-- - creator submissions must include a fixed music genre
...
drop function if exists public.listen_bar_block_promotion_capacity_eviction();
...
'AIPOGER Bar Heartbreak rotation. Current rule: each genre has a 36-track public pool; capacity eviction removes at most 3 low-reaction oldest non-honor overflow tracks inside overfull genre pools.';

...
- Choice 策展身分必須明確保存：一般 Showtime 創作者固定以自己的個人資料頭像與名稱發布；只有 owner 可逐期選擇 `官方 AIPOGER` 或 `愛波哥` 身分。官方使用品牌識別，愛波哥使用 owner 個人資料，不得用標題推測。Choice 卡片固定顯示週次日期、策展者 Choice 與當期自訂標題；推薦文章是長文欄位，直接顯示在卡片摘要與公開頁標題旁，不再另開文章 HUD。歌單只保留一個圖示：桌機 hover / focus、手機點擊顯示唯讀排序 HUD，不得在 HUD 放單曲播放或「全部播放」；播放只能由卡片原本播放鈕或 Choice 公開頁控制。Choice 提供歌單層級評論，登入者可留言與刪除自己的留言，其他留言可檢舉。發布與撤回必須同步保存畫面上的週次、標題、文章與 owner 身分，不能因未另按儲存而遺失。

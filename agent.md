...
- Choice 策展身分必須明確保存：一般 Showtime 創作者固定以自己的個人資料頭像與名稱發布；只有 owner 可逐期選擇 `官方 AIPOGER` 或 `愛波哥` 身分。官方使用品牌識別，愛波哥使用 owner 個人資料，不得用標題推測。Choice 卡片固定顯示週次日期，文章圖示開獨立 HUD 完整閱讀，分享圖示保持可見；歌單只保留一個圖示並開曲目 HUD，HUD 必須提供一次啟動整份順播的「全部播放」。推薦文章是長文欄位，不得退化成單行短評。
- 2026-07-15 Choice 分享頁：每份已發布 Choice 都要有自己的 `/choice/{id}?kind=official|creator` 公開連結；開啟後可直接逐首播放、全部順播、拖曳進度、上一首／下一首、調整音量、按歌單愛心收藏與分享，不可只導回 `/rank` 或顯示無法播放的靜態歌單。登入使用者收藏的 Choice 由 `/api/choice/saved` 讀取並在 Profile 顯示，可直接取消收藏；這是歌單層級收藏，不改歌曲 Heart、歌曲總愛心或歌曲收藏。

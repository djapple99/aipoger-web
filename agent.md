...
- 探索 AI 音樂的新歌曝光不做獨立 `最新上架` / `New Arrivals` / `72 小時新歌` 橫列、路由、看更多或 NEW 分類。符合展示條件的新歌以 `created_at`（不可用 `updated_at`）在 72 小時內排到自己的 genre lane 最前面；含新歌的 lane 依最新新歌 `created_at desc` 前推，沒有新歌的 lane 維持現行 11 類順序。同類型收合前 6 張中，同創作者最多一首 fresh work，其他仍留在該類型的看更多；完整展開仍是新歌優先、再依公開正向反應與時間排序。視覺上，Explore 與傷心酒吧可對 `created_at` 未滿滾動 7 x 24 小時的作品顯示 `NEW`；這不延長 72 小時排序保護，也不可改用 `updated_at`。Explore 的 `NEW` 固定封面左上，紅色 `接戰` 維持右上；不要使用 `Weekly`，避免與 AIPOGER Choice Weekly 混淆。

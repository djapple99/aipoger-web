...
- Choice 策展身分必須明確保存：一般 Showtime 創作者固定以自己的個人資料頭像與名稱發布；只有 owner 可逐期選擇 `官方 AIPOGER` 或 `愛波哥` 身分。官方使用品牌識別，愛波哥使用 owner 個人資料，不得用標題推測。Choice 卡片顯示推薦文章摘要，文章圖示開獨立 HUD 完整閱讀；歌單圖示只開曲目 HUD。

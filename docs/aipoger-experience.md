...
- Upload genre suggestions sit in a compact orange panel between audio selection and metadata fields. Disclose the short excerpt analysis before choosing a file. Loading, no suggestion, unavailable/retry and self-selection states are localized in zh/en/ja/ko. Every recommendation has its own `採用建議` button; `自行選擇` dismisses/cancels analysis. Neither receiving suggestions nor reading embedded tags changes the genre field. Keep the required manual menu usable throughout analysis; author confirmation controls the final genre. This addition and the three new genres are implemented locally, pending release.

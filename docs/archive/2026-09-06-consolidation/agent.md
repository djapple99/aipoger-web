...
> 2026-09-06 文件分層：本檔保留協作背景／舊速查，產品現況請先讀 [文件入口](docs/README.md) 與 `docs/aipoger-product-rules.md`。舊 24H 入口、過期套件版本、未完成待辦及只選 Showtime 的 Choice 摘要不能視為當前實作。Q Crash 與耳朵蟲現況以產品規則為準。
...
- 公開音樂頁的愛心公開資訊只顯示總愛心數；不要顯示收藏人數、誰收藏、或把收藏管理放到前台。Heart 按鈕可以為已登入使用者亮起，表示今天的 Heart 仍有效；按 Heart 等同同步收藏，再按一次就取消今天的 Heart 與收藏，總數要同步扣回。自己的收藏仍可在 Profile / 後台管理。探索 AI 音樂與傷心酒吧同一首歌必須讀寫同一個 `listen_bar_tracks` 總愛心數，不可分成兩套統計。
- 探索 AI 音樂底部 mini player 要有可拖曳播放進度條、時間標示與「歌詞」HUD；歌詞 HUD 在播放器上方彈出，旁邊提供可拖動的歌詞瀏覽滑桿，不要把歌詞展開到作品卡或列表中。
- 探索 AI 音樂的新歌曝光不做獨立 `最新上架` / `New Arrivals` / `72 小時新歌` 橫列、路由、看更多或 NEW 分類。`created_at` 未滿滾動 7 x 24 小時的作品同時取得 `NEW` 標示與排序前推：類型內所有 `NEW` 依 `created_at desc` 排在舊歌之前；含 `NEW` 的 lane 也依該類型最新上架時間前推，沒有 `NEW` 的 lane 維持現行 11 類順序。不可用 `updated_at` 讓舊歌重新上浮。同類型收合前 6 張中，同創作者最多一首 `NEW`，其他仍留在該類型的看更多；完整展開仍是 `NEW` 優先、再依公開正向反應與時間排序。Explore 的 `NEW` 固定封面左上，紅色 `接戰` 維持右上；不要使用 `Weekly`，避免與 AIPOGER Choice Weekly 混淆。
...
- Battle Pool 桌機 `VS` 必須有足夠視覺張力並清楚位於兩位角色之間；手機仍不顯示。登入頭像 Dock 預設停在右上安全位置，拖曳由全視窗 pointer 事件接管並以邊緣相對座標持久化，放手、重新整理或改變視窗寬度後不可回跳或漂到頁面中央。
...
- Choice 策展身分必須明確保存：一般 Showtime 創作者固定以自己的個人資料頭像與名稱發布；只有 owner 可逐期選擇 `官方 AIPOGER` 或 `愛波哥` 身分。官方使用品牌識別，愛波哥使用 owner 個人資料，不得用標題推測。Choice 卡片以創作者實際填寫的當期標題為主，策展者名稱與週次日期分開顯示，不得再拼成重複的 `策展者 Choice｜當期標題`；推薦文章直接顯示在卡片摘要與互動歌單 HUD 標題旁，不另開文章 HUD。歌單維持一個圖示：桌機 hover / focus 可快速預覽排序，點擊後開啟完整互動 HUD；手機直接點擊開 HUD。互動 HUD 每首歌提供既有歌曲收藏愛心與播放，底部提供「全部播放」和「開啟完整分享頁」，所有播放共用底部播放器。Choice 提供歌單層級評論，登入者可留言與刪除自己的留言，其他留言可檢舉。發布與撤回必須同步保存畫面上的週次、標題、文章與 owner 身分，不能因未另按儲存而遺失。
- 2026-07-16 Choice 分享頁：每份已發布 Choice 都要有自己的 `/choice/{id}?kind=official|creator` 公開連結與明顯的「回到 Showtime」。公開頁可逐首播放、全部順播、拖曳進度、上一首／下一首、調整音量、按歌單愛心收藏、評論與分享；不可只導回 `/rank` 或顯示無法播放的靜態歌單。登入使用者收藏的 Choice 由 `/api/choice/saved` 讀取並在 Profile 顯示，可直接取消收藏；這是歌單層級收藏，不改歌曲 Heart、歌曲總愛心或歌曲收藏。登入後全站頭像圓標可自由拖曳並記住位置，避免擋住 Choice 與其他頁面操作。
...
- 每日推薦歌 / Daily Spotlight 已退役：不再是傷心酒吧、Explore、`/admin/listen-bar` 或社群發布工作流，也不建立新的單曲 Spotlight 替代品。`/today` 只保留歷史連結相容並固定導向 `/rank?lang=<lang>#choice-weekly`；`/listen-bar?spotlight=...` 必須正常開啟一般公播。`AIPOGER Choice` 是唯一人為策展方向，每週 5-10 首、不排名、可跨類型；`/admin/showtime` 是 Choice 的主要勾選與發布工作台，`/admin/choice` 保留為直接管理入口，`/admin/social` 仍是唯一社群草稿、批准與手動發布中控台。

...
- The Suno library contains 118 unique free prompt moves, 21 unique lyric moves, at least 80 unique normalized genre terms, and six production-flow steps. The lyric library includes the five-layer basic vocal direction cues for delivery, character and tone, effects, placement, and transitions. The 118 prompts include the original 50 Studio Mastering prompts plus 45 Beatport screenshot expansion prompts, alongside the core prompt moves. Every move has bilingual title, summary, use case, copy text, source attribution, and an official / field-tested / version-sensitive evidence label.

...
- `/admin/listen-bar` defaults to the `全部上架` active/on-air view, has no duplicate `隱藏下架` filter, and provides the explicit `只看下架` view with restore. It shows `NEW` for `created_at` within seven rolling days and persists the owner-only external-promotion checkbox without changing `promoted_at`.

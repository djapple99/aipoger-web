...
- Showtime begins with a dedicated cover-led `AIPOGER CHOICE` shelf at `#choice-weekly`, before the Showtime heading and filters. Published Choice cards use the recommender identity as a square cover, show date plus `curator Choice | authored title`, display the stored recommendation article excerpt inline, and expose card playback, share, comments, a collection-level toggleable save, and one tracklist icon. The tracklist HUD is read-only: desktop hover/focus and mobile tap reveal order, with no row playback or HUD `Play all`. Do not show `CURATOR SETS`, circular curator avatars, a standalone article HUD, or `由創作者選出他們心目中的歌單`.

...
- The Suno library contains 42 unique prompt moves, 18 unique lyric moves, at least 80 unique normalized genre terms, and six production-flow steps. The 42 prompts include 18 genre-specific Studio Mastering prompts plus one general studio baseline. Every move has bilingual title, summary, use case, copy text, source attribution, and an official / field-tested / version-sensitive evidence label.
...
- The server-rendered public starter and five-question FAQ are localized for Chinese, English, Japanese, and Korean. They remain high-level, expose none of the complete 1,518-entry member index, and match the localized `TechArticle` / `FAQPage` JSON-LD plus canonical, hreflang, Open Graph, and Twitter metadata.

...
- `Prompt 招式庫` and `歌詞控制` are the Bible's prompt-first practice surface. They adapt nine owner-provided Suno PDFs, DOCX files, a genre screenshot, the credited NuNaught community prompting guide, the expanded Studio Mastering prompt pack, and a modular instrument-tone pack into 158 bilingual prompt moves, 21 bilingual lyric-control moves, 93 normalized genre terms, a six-step production workflow, and the large inspiration indexes below.
...
- This official owner Choice remains separate from creator-published Choice. Any signed-in creator may use `/profile/choice` to create one Choice collection per Monday week; a Showtime-certified work is not required. The selection pool is the same cross-creator pool of currently public Showtime works and eligible 30-day new releases, and each published collection must contain 5-10 works. Each creator can manage only their own drafts and published collections; a published creator Choice has its own share page and never replaces the official weekly Choice at `#choice-weekly`.
- Creator Choice is open curation, not a platform ranking or recognition gate. Collection-level hearts, saves, comments and shares may be used for feedback and discovery, but never alter a song's Heart total, battle record, Showtime status, or Explore order. The owner may remove a creator Choice from the admin Choice library when its curation is unsuitable; that removes the playlist and its collection interactions only, and does not alter any selected song.

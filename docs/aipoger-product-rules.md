...
- `AIPOGER Choice` is the first public content shelf on `/rank`, before the Showtime heading, filters, and genre catalog. Render every published Choice as a compact square, cover-led editorial card. Creator Choices always use the creator's current profile avatar and display name. Only the owner workbench may explicitly choose per collection between the official AIPOGER brand identity and the owner's personal `愛波哥` identity; persist this choice as data and never infer it from the title. Cover play starts the playlist in order; separate tracklist, article, share, and toggleable Choice-save actions remain visible. The editor's `intro` is the recommendation article excerpt, and its article action opens a dedicated readable HUD rather than mixing the full article into the tracklist dialog. The title is the large designed `AIPOGER CHOICE` wordmark only: do not show `CURATOR SETS`, circular avatars, or the redundant public explanation `由創作者選出他們心目中的歌單`. Choice saves are collection-level and do not reuse song Hearts, song totals, or the song 24-hour cooldown. Choice remains non-ranked and is never an automated weekly winner.

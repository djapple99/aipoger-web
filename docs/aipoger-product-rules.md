...
- `Prompt 招式庫` and `歌詞控制` are the Bible's prompt-first practice surface. They adapt nine owner-provided Suno PDFs, DOCX files, a genre screenshot, and the credited NuNaught community prompting guide into 23 bilingual prompt moves, 18 bilingual lyric-control moves, 93 normalized genre terms, a six-step production workflow, and the large inspiration indexes below.
...
- NuNaught's Reddit `Suno Prompting Guide` and Apache-2.0 `suno-songwriting` skill are credited community sources. Their two-field routing, vocal-identity, instrument-role, section-density, fusion-builder, combined-bracket, cue-count, and singability patterns are normalized into original bilingual practice cards instead of republishing the source article. Community heuristics such as 4-8 enriched cues or a target prompt length remain field-tested/version-sensitive guidance rather than official Suno limits.

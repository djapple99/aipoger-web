...
- Confirmed 2026-09-20, local implementation pending release: add `Children's Music 兒歌`, `Latin / Reggae 拉丁雷鬼` (Latin, Reggae and Reggaeton together), and `Cinematic 電影配樂` after the original 11 values. Preserve existing `/l/1` through `/l/11` links; new shares are `/l/12` through `/l/14`. Existing songs and frozen chart entries are not reclassified.

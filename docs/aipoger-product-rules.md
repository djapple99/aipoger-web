...
- Q Crash stores one unified Track contract: `source_type` is `suno` or `upload`, `source_url` stores the public Suno link or the private Storage object key, and `title`, `creator`, `cover_url`, and optional `duration_seconds` describe the work. Suno audio is never downloaded, fetched, transcoded, copied, or cropped; the card keeps a direct Suno link fallback. Uploaded audio stays as the original full file.

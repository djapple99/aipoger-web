...
- `AIPOGER Choice` 是唯一人為策展訊號：每週 5-10 首、不排名、可跨類型。Choice 主 CTA 是 `/rank?lang=zh#choice-weekly`；owner 主要從 `/admin/showtime` 的作品勾選模式建立週次、挑選公開 Showtime 認證作品或上架未滿滾動 7 x 24 小時的公開新歌、排序、發布與撤回，`/admin/choice` 保留為相同資料的直接管理入口。新歌入選 Choice 不會取得 Showtime 認證。
...
- The `錄音室 Mastering` prompt category contains 49 genre-, culture-, or era-specific Style prompts plus one general studio baseline. Its searchable range includes distinct jazz branches, 1950s–2000s recording aesthetics, global forms, orchestral writing, rock, blues, country, gospel, pop, and electronic production. These are generation directions with version-sensitive behavior, not guarantees of post-production mastering; users should export and measure the final master separately.

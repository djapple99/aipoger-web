...
- `Prompt 招式庫` and `歌詞控制` are the Bible's prompt-first practice surface. They adapt seven owner-provided Suno PDFs, DOCX files, and a genre screenshot into 18 bilingual prompt moves, 16 bilingual lyric-control moves, 93 normalized genre terms, and a six-step production workflow.

...
type TrackVisibilityFilter = "all" | "active" | "hidden" | "removed";
...
export default function ListenBarAdminPage() {
  ...
  const pauseOtherAdminAudio = useCallback((currentAudio: HTMLAudioElement) => {
    document.querySelectorAll<HTMLAudioElement>("[data-listen-bar-admin-audio]").forEach((audio) => {
  ...
  const renderedTracks = useMemo(() => {
    ...
    const filteredTracks = displayTracks.filter((track) => {
      ...
      if (trackGenreFilter !== "all" && normalizedGenreForSelect(track.genre) !== trackGenreFilter) return false;
  ...
  const handleAudioChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && !isAllowedListenBarAudioFile(file)) {
      setAudioFile(null);
      setAudioPreview("");
      setError("請使用 MP3、M4A、AAC 或 OGG 壓縮音檔，避免公播時卡頓。");
      return;
    }
    if (file && file.size > LISTEN_BAR_AUDIO_UPLOAD_MAX_BYTES) {
      setAudioFile(null);
      setAudioPreview("");
      setError(`音檔太大：${listenBarAudioSizeLabel(file)}。傷心酒吧單檔上限是 ${LISTEN_BAR_AUDIO_UPLOAD_MAX_LABEL}，請先轉成 MP3/M4A 或壓縮。`);
      return;
    }
    setAudioFile(file);
        ...
        <section className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            ...
            <div className="grid gap-3">
              ...
              {audioPreview && (
                <audio
                  ...
                  controls
                  data-listen-bar-admin-audio
                  preload="metadata"
                  src={audioPreview}
                  onPlay={(event) => pauseOtherAdminAudio(event.currentTarget)}
                >
                  <track kind="captions" />
                </audio>
              )}
          ...
          <section className="rounded-[1.4rem] border border-white/10 bg-white/[0.045] p-4 shadow-[0_20px_70px_rgba(0,0,0,0.4)] backdrop-blur md:p-5">
            ...
            <div className="mb-4 grid gap-3 rounded-2xl border border-white/10 bg-black/32 p-3">
              ...
              />
              <div className="grid gap-2 sm:grid-cols-[auto_auto_minmax(9rem,1fr)_minmax(9rem,1fr)_minmax(10rem,1fr)]">
                <button
                  type="button"

          ...
          <section className="mt-5 rounded-xl border border-cyan-200/25 bg-cyan-300/[0.055] p-4 sm:p-5">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-cyan-100/10 pt-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedChoice ? <button type="button" disabled={Boolean(choiceBusy)} onClick={() => void runChoiceAction("set_published", { collectionId: selectedChoice.id, isPublished: !selectedChoice.isPublished }, selectedChoice.id)} className={`rounded-full border px-4 py-2 text-xs font-black disabled:opacity-45 ${selectedChoice.isPublished ? "border-red-200/35 bg-red-500/10 text-red-100" : "border-cyan-100/50 bg-cyan-300 text-black"}`}>{choiceBusy === "set_published" ? "處理中" : selectedChoice.isPublished ? "撤回發布" : "發布 Choice"}</button> : null}
              </div>
            </div>

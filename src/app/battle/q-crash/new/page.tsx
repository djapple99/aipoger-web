...
import { ArrowLeft, Clock3, ExternalLink, Search, Swords, Upload, UserRoundPlus, X } from "lucide-react";
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_18%_12%,rgba(249,115,22,0.18),transparent_28%),radial-gradient(circle_at_82%_24%,rgba(34,211,238,0.10),transparent_30%),#050505] px-4 pb-16 pt-24 text-white md:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        {sourceType === "suno" && sunoUrl && isValidQCrashSunoUrl(sunoUrl) ? <p className="mt-4 text-right text-xs font-bold text-zinc-600"><ExternalLink className="mr-1 inline" size={13} />{isZh ? "Suno 連結將作為播放 fallback" : "The Suno link will remain the playback fallback"}</p> : null}
      </div>
    </main>
  );
}

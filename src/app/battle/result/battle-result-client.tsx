...
function BattleResultContent() {
  ...
  const resultBody = isOfficialBattleResult
    ...
    : lang === "zh"
      ? `這首「${winnerSong}」已分出勝負，但本場只有 ${audienceCount}/${officialAudienceMin} 名觀眾投票，不累計歌曲正式戰績。`
      : `"${winnerSong}" won the room, but only ${audienceCount}/${officialAudienceMin} audience voters joined. This stays unofficial and does not count toward official song stats.`;
        ...
        <section className="mx-auto w-full max-w-2xl lg:mx-0">
          ...
          {!isOfficialBattleResult ? (
            <p className="mt-4 inline-flex rounded-full border border-yellow-200/30 bg-yellow-300/10 px-3 py-1.5 text-xs font-black text-yellow-100">
              {lang === "zh" ? `正式戰績門檻：${officialAudienceMin} 名不同觀眾` : `Official result threshold: ${officialAudienceMin} distinct audience voters`}
            </p>
          ) : null}
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm font-bold leading-6 text-zinc-400">
                  {isOfficialBattleResult
                    ? lang === "zh"
                      ? "五角評分圖，呈現這首 Drop 的爆點分布。"
                      : "Signal radar showing this Drop's strongest points."
                    : lang === "zh"
                      ? "非正式戰果仍保留爆點分析，不累計正式戰績。"
                      : "Unofficial results keep the signal radar without counting toward official records."}
                </p>
              </div>
              ...
              <div className="grid min-w-0 grid-cols-[auto_1fr] items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-200/70">
                    {isOfficialBattleResult ? (lang === "zh" ? "正式對戰勝方" : "BATTLE WINNER") : lang === "zh" ? "非正式勝方" : "UNOFFICIAL WINNER"}
                  </p>

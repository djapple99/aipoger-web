...
const COPY = {
  zh: { month: "月份", style: "類型", all: "總榜 · 所有類型", search: "搜尋歌名或創作者", live: "本月進行中", final: "已結算", supporters: "支持人數", accumulating: "人氣累積中", empty: "本月人氣正在累積", noMatch: "沒有符合的歌曲", explore: "去探索音樂", loading: "正在讀取月榜", failed: "月榜暫時無法讀取，請稍後重試。", retry: "重新整理", playAll: "播放榜單", play: "播放", pause: "暫停", heart: "愛心支持並收藏", remove: "取消今日愛心與收藏", login: "請先登入，才能支持與收藏歌曲。", signIn: "登入", heartFailed: "愛心暫時無法更新，請重試。", rules: "月榜規則", close: "關閉", share: "分享歌曲", copied: "已複製", ruleText: "依台灣時間每月計算有效愛心支持。同一帳號每首歌每月最多計一次，排除作者自投；至少 3 位支持者才列名次，同票並列。月底結算保留歷史榜；已下架歌曲不再提供播放。Choice 收藏、播放次數與 Battle 戰績不計入月榜。", count: "位支持者" },
  en: { month: "Month", style: "Genre", all: "Overall · All genres", search: "Search song or creator", live: "This month · Live", final: "Final chart", supporters: "Supporters", accumulating: "Gaining support", empty: "This month's support is building", noMatch: "No matching songs", explore: "Explore Music", loading: "Loading monthly chart", failed: "The chart is unavailable. Please try again.", retry: "Refresh", playAll: "Play chart", play: "Play", pause: "Pause", heart: "Support and save song", remove: "Remove today's heart and save", login: "Sign in to support and save songs.", signIn: "Sign in", heartFailed: "Could not update your heart. Try again.", rules: "Chart rules", close: "Close", share: "Share song", copied: "Copied", ruleText: "Each Taiwan calendar month counts distinct accounts with an active song Heart, once per song. Creator self-support is excluded. Songs need at least 3 supporters to rank; equal counts share a rank. Monthly results are finalized and retained. Removed songs are not playable. Choice saves, plays and Battle results do not affect this chart.", count: "supporters" },
  ja: { month: "月", style: "ジャンル", all: "総合 · 全ジャンル", search: "曲名・クリエイターで検索", live: "今月 · 集計中", final: "確定ランキング", supporters: "支持者数", accumulating: "支持が集まっています", empty: "今月の支持を集計中", noMatch: "該当する曲はありません", explore: "音楽を探す", loading: "月間ランキングを読込中", failed: "ランキングを読み込めません。再試行してください。", retry: "更新", playAll: "ランキングを再生", play: "再生", pause: "一時停止", heart: "曲を支持・保存", remove: "今日のハートと保存を解除", login: "曲の支持・保存にはログインしてください。", signIn: "ログイン", heartFailed: "ハートを更新できません。再試行してください。", rules: "ランキングのルール", close: "閉じる", share: "曲を共有", copied: "コピー済み", ruleText: "台湾時間の暦月ごとに、有効なハートを送った異なるアカウントを1曲につき1回集計します。作者本人の支持は除外し、3人以上で順位を表示。同数は同順位です。月末に確定し履歴を保存します。非公開曲は再生できません。Choiceの保存、再生回数、対戦成績は集計対象外です。", count: "人の支持" },
  ko: { month: "월", style: "장르", all: "종합 · 모든 장르", search: "곡명 또는 크리에이터 검색", live: "이번 달 · 집계 중", final: "확정 차트", supporters: "지지자 수", accumulating: "지지가 모이고 있어요", empty: "이번 달 지지를 집계 중입니다", noMatch: "일치하는 곡이 없습니다", explore: "음악 탐색", loading: "월간 차트 불러오는 중", failed: "차트를 불러올 수 없습니다. 다시 시도하세요.", retry: "새로고침", playAll: "차트 재생", play: "재생", pause: "일시정지", heart: "곡 지지 및 저장", remove: "오늘의 하트 및 저장 취소", login: "곡을 지지하고 저장하려면 로그인하세요.", signIn: "로그인", heartFailed: "하트를 업데이트하지 못했습니다. 다시 시도하세요.", rules: "차트 규칙", close: "닫기", share: "곡 공유", copied: "복사됨", ruleText: "대만 시간 기준 매월 유효한 곡 하트를 보낸 서로 다른 계정을 곡당 한 번 집계합니다. 창작자의 자기 지지는 제외하며, 3명 이상부터 순위를 표시합니다. 동수는 공동 순위입니다. 월말에 결과를 확정하고 보관합니다. 비공개 곡은 재생할 수 없습니다. Choice 저장, 재생 횟수, 대전 성적은 집계하지 않습니다.", count: "명의 지지" },
};
...
export default function MonthlyChart({ lang }: { lang: Lang }) {
  ...
  const ranked = visible.filter((track) => track.rank !== null);
  const accumulating = visible.filter((track) => track.rank === null);
